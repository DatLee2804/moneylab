    ...
    <DashboardLayout role="admin" title="Quản lý người dùng">
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="bg-white rounded-[40px] border border-[#E8E3D9] shadow-sm overflow-hidden">
          <div className="p-8 border-b border-[#E8E3D9] flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h3 className="text-xl font-black text-[#1C221F] uppercase tracking-tight">Cơ sở dữ liệu Người dùng</h3>
              <p className="text-sm text-gray-500 font-bold uppercase tracking-widest text-[10px] mt-1">Quản lý thành viên hệ thống từ Database thực tế</p>
            </div>
            <div className="flex items-center space-x-3">
              <div className="relative">
                ...
                <input 
                  ...
                  className="pl-10 pr-4 py-2 bg-white border border-[#E8E3D9] rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#133E2B]/20 transition-all outline-none text-[#1C221F] placeholder:text-gray-600" 
                />
              </div>
              <button onClick={fetchUsers} className="p-2.5 bg-white text-gray-500 rounded-xl border border-[#E8E3D9] font-sans hover:text-[#1C221F] transition-colors">
                <Filter size={18} />
              </button>
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50">
                    <th className="px-8 py-5 text-[10px] font-black text-gray-500 uppercase tracking-widest">Thông tin chính</th>
                    <th className="px-8 py-5 text-[10px] font-black text-gray-500 uppercase tracking-widest">Vai trò</th>
                  ...
                  {filteredUsers.length === 0 ? (
                    ...
                    <tr key={u.id} className="hover:bg-white/5 transition-colors group">
                      <td className="px-8 py-6">
                        <div className="flex items-center space-x-3">
                          <div className="w-12 h-12 rounded-2xl bg-white text-[#1C221F] flex items-center justify-center font-black text-lg shadow-sm border border-[#E8E3D9]">{u.name.charAt(0)}</div>
                          <div>
                            <p className="text-sm font-black text-[#1C221F] group-hover:text-[#1C221F] transition-colors">{u.name}</p>
                      ...
                      <td className="px-8 py-6">
                        <span className={cn(
                          ...
                          u.role === 'manager' ? "bg-[#133E2B]/10 text-[#1C221F]" : 
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center justify-end space-x-2">
                          ...
                          <button 
                            ...
                            className="p-2.5 bg-white/5 text-gray-500 rounded-xl hover:bg-white/10 hover:text-[#1C221F] transition-all border border-[#E8E3D9]" 
              ...
              <form onSubmit={handleSendEmail} className="p-8 space-y-6">
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <input required type="text" value={emailForm.subject} onChange={(e) => setEmailForm({...emailForm, subject: e.target.value})} placeholder="Nhập tiêu đề..." className="w-full px-6 py-4 bg-white border border-[#E8E3D9] rounded-2xl text-sm font-bold focus:ring-2 focus:ring-[#133E2B]/20 transition-all outline-none text-[#1C221F]" />
                  </div>
                  <div className="space-y-1.5">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white rounded-[40px] overflow-x-hidden overflow-y-auto max-h-[90vh] shadow-2xl max-w-2xl w-full border border-[#E8E3D9] custom-scrollbar">
              <div className="p-10 bg-white text-[#1C221F] flex items-center justify-between sticky top-0 z-10 border-b border-[#E8E3D9]">
                ...
                <button onClick={() => setIsEditModalOpen(false)} className="p-3 hover:bg-white/10 rounded-2xl transition-all text-[#1C221F]"><X size={28} /></button>
              </div>

              <form onSubmit={handleUpdateUser} className="p-10 space-y-10">
                {/* Account Section */}
                <section>
                  <div className="flex items-center space-x-3 mb-6 border-l-4 border-[#133E2B] pl-4">
                    <ShieldCheck size={20} className="text-[#1C221F]" />
                    <h4 className="text-sm font-black text-[#1C221F] uppercase tracking-widest">Thông tin Tài khoản</h4>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-3">Họ và tên</label>
                      <input required type="text" value={editForm.name} onChange={(e) => setEditForm({...editForm, name: e.target.value})} className="w-full px-6 py-4 bg-white border border-[#E8E3D9] rounded-2xl text-sm font-bold focus:ring-2 focus:ring-[#133E2B]/20 transition-all outline-none text-[#1C221F]" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-3">Email liên hệ</label>
                      <input required type="email" value={editForm.email} onChange={(e) => setEditForm({...editForm, email: e.target.value})} className="w-full px-6 py-4 bg-white border border-[#E8E3D9] rounded-2xl text-sm font-bold focus:ring-2 focus:ring-[#133E2B]/20 transition-all outline-none text-[#1C221F]" />
                    </div>
                  </div>
                </section>
                ...
                <section>
                  <div className="flex items-center space-x-3 mb-6 border-l-4 border-amber-400 pl-4">
                    ...
                    <h4 className="text-sm font-black text-[#1C221F] uppercase tracking-widest">Thông tin Ngân hàng</h4>
                  </div>
                  <div className="p-8 bg-amber-400/5 rounded-3xl border border-amber-400/10 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div className="space-y-1.5">
                          ...
                          <input type="text" value={editForm.bankName || ''} onChange={(e) => setEditForm({...editForm, bankName: e.target.value})} className="w-full px-5 py-4 bg-white rounded-xl text-sm font-black text-gray-600 border border-[#E8E3D9]" />
                       </div>
                       <div className="space-y-1.5">
                          <p className="text-[10px] font-black text-amber-400/40 uppercase tracking-widest pl-1">Chủ tài khoản</p>
                          <input type="text" value={editForm.bankOwner || ''} onChange={(e) => setEditForm({...editForm, bankOwner: e.target.value})} className="w-full px-5 py-4 bg-white rounded-xl text-sm font-black text-gray-600 border border-[#E8E3D9]" />
                       </div>
                       <div className="space-y-1.5 md:col-span-2">
                          <p className="text-[10px] font-black text-amber-400/40 uppercase tracking-widest pl-1">Số tài khoản</p>
                          <input type="text" value={editForm.bankAccount || ''} onChange={(e) => setEditForm({...editForm, bankAccount: e.target.value})} className="w-full px-5 py-4 bg-white rounded-xl text-lg font-mono font-black text-[#1C221F] border border-[#E8E3D9] tracking-widest" />
                       </div>
                    </div>
                  </div>
                </section>
