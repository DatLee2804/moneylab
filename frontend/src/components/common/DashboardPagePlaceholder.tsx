...
export const DashboardPagePlaceholder = ({ role, title, description, icon }: DashboardPagePlaceholderProps) => {
  return (
    <DashboardLayout role={role} title={title}>
      ...
      >
        <div className="w-20 h-20 bg-[#133E2B]/10 rounded-[32px] flex items-center justify-center text-[#1C221F] mb-8 animate-pulse">
          {icon}
        </div>
        <h2 className="text-2xl font-black text-[#0f172a] dark:text-[#1C221F] mb-4">{title} Đang Phát Triển</h2>
        <p className="text-gray-500 max-w-md mx-auto leading-relaxed font-medium">
          {description}
        </p>
        <div className="mt-10 flex gap-4">
          <div className="h-2 w-12 bg-[#133E2B] rounded-full"></div>
